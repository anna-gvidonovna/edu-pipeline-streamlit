...
let disc_type = get-discipline-type(discipline_code)
  ...
  let discipline_here = if discipline_code.starts-with("Б2") { "практике" } else { "дисциплине" }
  ...
  table(
   ...
   align(center)[
    #upper[
      *фонд оценочных средств по #discipline_here*
    ] \
    #upper[
...
if prepared.pk.len() > 0 [
  ...
  heading[Формы и оценочные материалы текущего контроля успеваемости и промежуточной аттестации]

heading("Текущий контроль успеваемости по разделам дисциплины", level: 2)
// Использование с разными формами контроля:
let control_forms = (
  [устный опрос \ и свободная дискуссия в аудитории],
  [устный опрос \ и свободная дискуссия в аудитории],
  [устный опрос \ и свободная дискуссия в аудитории],
  [устный опрос \ и свободная дискуссия в аудитории],
  [устный опрос \ и свободная дискуссия в аудитории],
...
v(1em)
create-current-control-table(topics, control_forms: control_forms)
 v(1em)
 [Текущий контроль успеваемости осуществляется на основе:
- устной дискуссии в аудитории на семинарах;
- выполнения самостоятельной работы, предусмотренной учебным планом.
#v(0.5em)
В качестве оценочных средств текущего контроля предусмотрены: посещение занятий и активность на обсуждениях; участие в тематических дискуссиях.]
[
#format-discussion-topics(current_topics, spacing: 0.5em)
]

pagebreak()
heading("Промежуточная аттестация", level: 2)
[
]
[#v(0.5em)Промежуточная аттестация по дисциплине проводится в форме тестирования. Тестирование проверяет освоение у обучающихся планируемых результатов обучения, соотнесенных с планируемыми результатами освоения образовательной программы.]

// Получаем код текущей дисциплины
  let current_discipline_code = module_desc.at(var_cycle).at("discipline_code", default: "")

  v(1em)

  // Вызываем новую функцию для создания таблицы
  create-assessment-table(
    current_discipline_code, // "Б1.В.01" и т.д.
    assessment_csv,          // Файл 3.4.1
    indicators_csv           // Файл 3.3.1 (для текста компетенций)
  )

  pagebreak()
[#heading("Шкала оценки текущего контроля успеваемости и промежуточной аттестации", level: 2)
#v(1em)
#align(center)[
  #table(
    columns: (1fr, 1fr, 3fr),
    ...
    align(center)[#text(size: 9pt)[диапазон баллов]], 
    align(center)[#text(size: 9pt)[оценка по ECTS]], 
    align(center)[#text(size: 9pt)[качественная оценка]],
    [92–100], [A], [5 (Отлично)],
    [82–91],  [B], [5 (Отлично)],
    [62–81],  [C], [4 (Хорошо)],
    [52–61],  [D], [3 (Удовлетворительно)],
    [42–51],  [E], [3 (Удовлетворительно)],
    [0–41],   [F], [2 (Неудовлетворительно)],
  )
]
#v(1em)]
