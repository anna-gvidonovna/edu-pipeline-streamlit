...
#let finals(
  ...
  let gia_full_cap = if is_accredited { "Государственной итоговой аттестации" } else { "Итоговой аттестации" }
  let gia_short = if is_accredited { "государственная итоговая аттестация" } else { "итоговая аттестация" }
  let gia_short_cap = if is_accredited { "Государственная итоговая аттестация" } else { "Итоговая аттестация" }
  let gia_adj = if is_accredited { "государственной" } else { "" }
  let gek = if is_accredited { "государственной экзаменационной комиссии" } else { "экзаменационной комиссии" }
  let gek_cap = if is_accredited { "Государственной экзаменационной комиссии" } else { "Экзаменационной комиссии" }
  let gek_short = if is_accredited { "ГЭК" } else { "ЭК" }
...
v(1em)
 ...
 outline()
  ...
  [В соответствии с Федеральным законом от 29 декабря 2012 г. № 273-ФЗ «Об образовании в Российской Федерации»,  а также ФГОС ВО по направлению подготовки #direction (#level), приказ Министерства образования и науки России от #fgos_date № #fgos_number, освоение основных профессиональных образовательных программ высшего образования завершается #gia_full выпускников.

]

  heading("Цель и задачи " + gia_full, level: 2)

  [Целью #gia_full является установление уровня подготовки выпускника Университета к выполнению профессиональных задач и соответствия его подготовки требованиям Федерального государственного образовательного стандарта высшего образования по направлению подготовки #direction (#level).
#v(0.5em)
  #gia_short_cap направлена на решение следующих задач:
...
- оценить уровень подготовленности выпускников к самостоятельной профессиональной деятельности.
 ...
 heading("Формы аттестационных испытаний", level: 2)
  ...
  [Форма #gia_full по направлению подготовки #direction (профиль) #program_name: 
...
heading("Трудоемкость " + gia_full, level: 2)

  [Общая трудоемкость в #last_sem_info.semester семестре составляет #last_sem_info.credits з.е. #v(0.5em) К #gia_full допускается обучающийся, не имеющий академической задолженности и в полном объеме выполнивший учебный план или индивидуальный учебный план.]
...
[Члены #gek_short оценивают ВКР, исходя из степени раскрытия темы, самостоятельности и
...
рецензент и сами члены #gek_short. #v(0.5em)Результаты определяются оценками «отлично», «хорошо», «удовлетворительно»,
«неудовлетворительно».
#pagebreak()
Критерии оценки ВКР:
#v(0.5em)
#table(
  ...
  [«Отлично»], [
    ...
    - ответы на вопросы членов #gek_short показывают глубокое знание исследуемой проблемы, подкрепляются ссылками на соответствующие нормативно-правовые акты, литературные источники, выводами из ВКР, демонстрируют самостоятельность и глубину изучения проблемы студентом;
  ...
  [«Хорошо»], [
    ...
    - ответы на вопросы членов #gek_short показывают хорошее владение материалом, нормативно-правовыми актами действующего законодательства, подкрепляются выводами из ВКР, показывают самостоятельность и глубину изучения проблемы студентом;
  ...
  [«Удовлетворительно»], [
    ...
    - ответы на вопросы членов #gek_short носят не достаточно полный и аргументированный характер, не раскрывают до конца сущности вопроса, слабо подкрепляются выводами из ВКР, показывают недостаточную самостоятельность и глубину изучения проблемы студентом;
  ...
  [«Неудовлетворительно»], [
    ...
    - ответы на вопросы членов #gek_short носят неполный характер, не раскрывают сущности вопроса, не подкрепляются выводами и расчетами из ВКР, показывают недостаточную самостоятельность и глубину изучения проблемы студентом;
...
let vkr_topics_raw = title_csv.at(title_n).at("vkr_topics", default: "")
let vkr_topics_custom = vkr_topics_raw.split(";").map(item => str(item).trim()).filter(item => item != "")
let vkr_topics_default = (
  "Проектирование аграрного исследовательского и образовательного комплекса",
...
let vkr_topics = if vkr_topics_custom.len() > 0 { vkr_topics_custom } else { vkr_topics_default }

heading("Примерная тематика ВКР", level: 3)
[
  #for topic in vkr_topics [
    - #topic
  ]
]
...
[ Защита выпускной квалификационной работы проводится в установленное время на заседании #gek. Кроме членов комиссии на защите должен присутствовать руководитель выпускной квалификационной работы.
  #v(0.5em)
  ...
  #v(0.5em)
  Перед началом защиты председатель #gek знакомит студентов с порядком проведения защиты, секретарь комиссии представляет студента и тему его выпускной квалификационной работы.
  #v(0.5em)
  Защита начинается с доклада студента по теме выпускной квалификационной работы, на который отводится до 15 минут.
  #v(0.5em)
  После завершения доклада члены #gek задают студенту вопросы как непосредственно связанные с темой выпускной квалификационной работы, так и с проблемой, решению которой посвящена работа. При ответах на вопросы студент имеет право пользоваться своей работой.
  #v(0.5em)
  ...
  #v(0.5em)
  В случае отсутствия руководителя и/или рецензента председатель #gek зачитывает отзыв и/или рецензию на выпускную квалификационную работу.
  #v(0.5em)
  ...
  #v(0.5em)
  Общее время защиты выпускной квалификационной работы с учетом дополнительных вопросов членов #gek должно составлять не более 30 минут.
  #v(0.5em)
  Защита выпускных квалификационных работ оформляется протоколом. Протоколы подписываются членами #gek и председателем #gek.
]

pagebreak()

heading("Учебно-методическое и информационное обеспечение " + gia_full, level: 1)
...
heading("Материально-техническое обеспечение " + gia_full, level: 1)
mats_tech
